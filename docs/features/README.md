...
- [`incoming-message-routing.md`](./incoming-message-routing.md) — Strict BullMQ ingestion with bounded lock-contention retries, accurate failure logs, sensitive-command bypasses and snapshot-bound enabled expense, option, cancellation, undo-offer, request-only retry, and recovery dispatch shared by Telegram and WhatsApp.
