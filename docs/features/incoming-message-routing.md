...
- Exact undo confirmation and exact `reconfigurar` now join those sensitive bypasses. `EXPENSE_CORRECTING/default` can be projected only from a validated correction payload, but Phase 1 control proposals remain observation/policy evidence and cannot cancel, delete, retry, reconfigure, or transition the FSM.
