...
- `DetectCategories.execute(input: DetectCategoriesInput): Promise<void>` — orchestrates token retrieval, column lookup, category reading, and user messaging.
...
- `user_categories` — will store the confirmed vocabulary in a future iteration.
- `spreadsheet_configs` — provides `fileId`, `sheetName`, and `provider`.
- `column_mappings` — provides the index of the category column for the active spreadsheet.

## Tests

- `src/application/use-cases/spreadsheet/DetectCategories.spec.ts` — covers detection, default fallback, and missing-config path.
...
- `src/interfaces/workers/message.worker.spec.ts` — covers `ONBOARDING_CATEGORIES` delegation and fallback.
