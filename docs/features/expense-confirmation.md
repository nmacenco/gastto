...
- In enabled semantic mode, a natural retry request never appends. After revalidating the current revision, unexpired retry payload, successful presentation binding, and absence of an execution claim, it sends `Respondé *reintentar* para volver a guardar o *reconfigurar* para revisar la planilla.` and leaves the retry state unchanged. Only a later exact `reintentar` can consume the binding and make the single retry attempt.
- A natural reconfiguration request uses the same retry-context checks and may call only `StartSpreadsheetReconfigurationUseCase` with the captured precondition. It reuses the active Google configuration, transitions to `ONBOARDING_VALIDATING_ACCESS`, and runs eager access validation without replaying the retained expense.
...
- `semantic-router-control-flows.integration.spec.ts` proves request-only semantic retry preserves the bound payload, a later exact retry appends at most once, and semantic reconfiguration changes only the captured retry revision before eager validation.
