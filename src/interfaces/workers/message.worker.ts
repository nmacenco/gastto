...
// Retry policy (ADR-005): 3 attempts with exponential backoff 1s → 2s → 4s
const RETRY_POLICY = {
  attempts: 3,
  ...
  const worker = new Worker<ProcessMessageJobData>(
    ...
    async (job: Job<ProcessMessageJobData>) => {
      ...
      const conversationState =
        await opts.conversationRepo.findByUserId(userId);
...
function formatExpenseSummary(
  payload: import("../../application/use-cases/expense/RegisterExpense").ExpenseReviewPayload,
): string {
  const { extracted, resolvedDate, resolvedCategory } = payload;
  const categoryLabel = resolvedCategory ?? "❓ Sin categoría";
  const confidenceNote =
    extracted.confianzaCategoria === "baja" ? " (¿correcto?)" : "";
