...
describe('ClaudeAdapter', () => {
  ...
  describe('interpretCorrection', () => {
    ...
    it('maps a combined category/subcategory correction with untrusted parent context', async () => {
      ...
      const [init] = createMock.mock.calls[0] as [Record<string, unknown>];
      expect(init.system).not.toContain('ParentPromptAttack');
      expect(init.system).not.toContain('ChildPromptAttack');
      const messages = init.messages as Array<{ role: string; content: string }>;
      expect(messages[0]?.content).toContain('"subcategoryEnabled": true');
      expect(messages[0]?.content).toContain('"name": "ParentPromptAttack"');
      expect(messages[0]?.content.match(/ChildPromptAttack/g)).toHaveLength(3);
    });
    ...
    it('maps a child-only correction and rejects a missing child field', async () => {
      ...
      await expect(
        new ClaudeAdapter(API_KEY).interpretCorrection(
