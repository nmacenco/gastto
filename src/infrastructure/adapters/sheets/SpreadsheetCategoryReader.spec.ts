...
function buildMockPort(values: string[]): { port: SpreadsheetPort; getUniqueValues: ReturnType<typeof vi.fn> } {
...
describe('SpreadsheetCategoryReader', () => {
  it('normalizes and deduplicates values from the spreadsheet port', async () => {
    const { port, getUniqueValues } = buildMockPort(['  Comida  ', 'TRANSPORTE', 'comida', 'Servicios', '']);
