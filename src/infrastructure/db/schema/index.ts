...
export const messagingIdentities = pgTable(
  ...
  (t) => ({
    lookupIdx: index('idx_messaging_identities_lookup').on(t.channel, t.externalId),
    userIdx: index('idx_messaging_identities_user').on(t.userId),
    channelCheck: check('chk_channel', sql`${t.channel} IN ('telegram', 'whatsapp')`),
    uniqueIdentity: uniqueIndex('uq_channel_external').on(t.channel, t.externalId),
  }),
);
...
export const conversationStates = pgTable(
  ...
  (t) => ({
    ...
    stateCheck: check(
      ...
      sql`${t.currentState} IN ('IDLE','ONBOARDING_START','ONBOARDING_DRIVE','ONBOARDING_FILE','ONBOARDING_SHEET','ONBOARDING_VALIDATING_ACCESS','ONBOARDING_MAPPING','ONBOARDING_CATEGORIES','EXPENSE_RECEIVING','EXPENSE_CLARIFYING','EXPENSE_REVIEW','EXPENSE_CORRECTING','EXPENSE_SAVING','EXPENSE_SAVING_RETRY','EXPENSE_UNDO_CONFIRMING')`,
    ),
  }),
);
...
export const expenseQueue = pgTable(
  ...
  (t) => ({
    userPositionIdx: index('idx_expense_queue_user_position').on(t.userId, t.position),
    uniqueUserPosition: uniqueIndex('uq_user_position').on(t.userId, t.position),
    positionRange: check('chk_position_range', sql`${t.position} BETWEEN 1 AND 2`),
    channelCheck: check('chk_queue_channel', sql`${t.channel} IN ('telegram', 'whatsapp')`),
  }),
);
...
export const oauthTokens = pgTable(
  ...
  (t) => ({
    userProviderIdx: index('idx_oauth_tokens_user_provider').on(t.userId, t.provider),
    expiresIdx: index('idx_oauth_tokens_expires').on(t.accessTokenExpiresAt),
    uniqueUserProvider: uniqueIndex('uq_user_provider').on(t.userId, t.provider),
    providerCheck: check('chk_provider', sql`${t.provider} IN ('google', 'microsoft')`),
  }),
);
...
export const spreadsheetConfigs = pgTable(
  ...
  (t) => ({
    ...
    providerCheck: check('chk_sp_provider', sql`${t.provider} IN ('google', 'microsoft')`),
  }),
);
