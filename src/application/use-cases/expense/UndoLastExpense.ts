...
export class UndoLastExpenseUseCase {
  ...
  async execute(userId: string): Promise<UndoLastExpenseOutput> {
    ...
    try {
      ...
      await this.spreadsheetPort.deleteRow(config.fileId, last.sheetName, last.rowIndex);
      ...
      await this.logRepo.create(userId, 'EXPENSE_SAVE_FAILED', { phase: 'undo' }, errorType);
