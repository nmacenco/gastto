...
beforeEach(() => {
  ...
  findLatest.mockResolvedValue({
    id: 'expense-1', concepto: 'Café', monto: 4.5, moneda: 'EUR', sheetName: 'Gastos', rowIndex: 8,
    savedAt: new Date('2026-08-02T10:00:00Z'),
  });
  findConfig.mockResolvedValue({ provider: 'google', fileId: 'file-1' });
  findToken.mockResolvedValue({ accessTokenEnc: Buffer.from('token'), iv: Buffer.from('iv'), revokedAt: null, accessTokenExpiresAt: new Date(Date.now() + 60_000) });
...
describe('UndoLastExpenseUseCase', () => {
  it('deletes externally before atomically soft-deleting and auditing locally', async () => {
    const result = await buildUseCase().execute({ userId: 'user-1', action: 'request', immediateEligible: true });

    expect(result).toMatchObject({ status: 'deleted', expense: { id: 'expense-1', concepto: 'Café' } });
    expect(deleteRow).toHaveBeenCalledWith('file-1', 'Gastos', 8);
    expect(softDeleteWithAudit).toHaveBeenCalledWith('expense-1', 'user-1', expect.objectContaining({ row: 8 }));
  ...
  it('does not mutate local state or emit a deletion audit when Sheets rejects the deletion', async () => {
    deleteRow.mockRejectedValue(new SpreadsheetError('Google Sheets API error during row deletion: HTTP 403'));

    await expect(buildUseCase().execute({ userId: 'user-1', action: 'request', immediateEligible: true }))
      .resolves.toEqual({ status: 'deletion_failed', errorType: 'AUTH_ERROR' });

    expect(softDeleteWithAudit).not.toHaveBeenCalled();
    expect(logCreate).toHaveBeenCalledWith('user-1', 'EXPENSE_SAVE_FAILED', { phase: 'undo' }, 'AUTH_ERROR');
  });

  it('returns confirmation_required without creating a spreadsheet port when immediate undo is unavailable', async () => {
    await expect(buildUseCase().execute({ userId: 'user-1', action: 'request', immediateEligible: false }))
