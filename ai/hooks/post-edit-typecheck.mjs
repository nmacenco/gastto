...
console.log('[Hook:TypeCheck] Waiting for input...');
...
process.stdin.on('end', () => {
  try {
    ...
    if (filePath && /\.(ts|tsx)$/.test(filePath)) {
      console.error('[Hook:TypeCheck] Executing on:', path.basename(filePath));
      const resolvedPath = path.resolve(filePath);
      if (!fs.existsSync(resolvedPath)) {
        console.error('[Hook:TypeCheck] ✗ File not found');
      ...
      if (fs.existsSync(path.join(dir, 'tsconfig.json'))) {
        console.error('[Hook:TypeCheck] Running tsc --noEmit...');
        try {
          ...
          console.error('[Hook:TypeCheck] ✓ No TypeScript errors');
        } catch (err) {
          ...
          if (relevantLines.length > 0) {
            console.error(
              '[Hook:TypeCheck] ✗ TypeScript errors in ' + path.basename(filePath) + ':'
            );
            relevantLines.forEach(line => console.error(line));
          } else {
            console.error(
              '[Hook:TypeCheck] ✓ No errors in this file (but project has other errors)'
            );
          }
        }
      } else {
        console.error('[Hook:TypeCheck] ✗ No tsconfig.json found');
      }
    }
  } catch {
