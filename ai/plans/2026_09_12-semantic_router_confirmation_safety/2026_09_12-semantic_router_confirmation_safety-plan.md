...
- [x] Implement validated review binding, legacy normalization, and conditional presentation completion in review creation, clarification completion, correction, zero/high-amount steps, queue advance, and re-presentation paths.
- [x] Implement the compact callback codec and strict bound/legacy DTO union through Telegram parser, normalized payload, incoming queue, RouteIncomingMessage, processing queue, worker, presenter, and application resolver. Validate before global callback cancellation; preserve identity checks, deduplication, and acknowledgments.
- [x] Extend action/reply resolver inputs with the captured state precondition and deterministic authorization evidence; return explicit stale/expired/unbound/invalid/in-progress outcomes and load the authorized payload from persistence.
- [x] Implement the exact financial command policy above and reject pre-presentation queued text using captured channel time. Keep mixed affirmative/correction messages in contextual correction without adding model-based authorization.
- [x] Implement the two-stage expiration policy with a new review binding during grace, safe legacy re-presentation, no stale-click TTL refresh, and the application-owned guidance copies.
- [x] Extend parser/DTO/presenter/resolver/worker tests and add `expense-confirmation-context.e2e.spec.ts`: original 30 EUR review, correction to 35 EUR, old confirm/cancel/correct all rejected, new confirm appends 35 once; same-state replacement; duplicate delivery with distinct callbacks; forged identity; malformed/versioned/legacy callbacks; legacy JSONB; expired before sweep; grace then second expiry; queued affirmatives; failed presentation; zero/high-amount stage replay; queue overflow leaves binding and draft intact.
- [x] Test command normalization as public policy with all exact vocabulary and negative examples `sí, pero cambia el importe a 25`, `no confirmo`, `si ok`, quoted `"si"`, emoji-decorated commands, internal punctuation, conditions, and prompt injection. Assert zero append/delete for rejected authorization and no loss of the original text.
- [x] Update confirmation, correction, summary-review, incoming-routing, cancellation, and state-management docs plus feature index and FSM payload documentation with implemented contracts and compatibility.
- [x] Run `pnpm test` including the complete callback-to-application regression with real core policy and mocked messaging/LLM/spreadsheet boundaries.
- [x] Run `pnpm run lint` and `pnpm run typecheck` to verify linting and typechecking. Fix issues if any.
- [x] Ask the user if they want to review the changes before continuing, or proceed directly with the next phase.

#### Completion evidence (2026-09-13)

- Review payloads now persist a random 128-bit base64url operation ID, positive review revision, and nullable successful-presentation timestamp. New, corrected, zero-stage, queued, grace-period, legacy, and stale-response presentations participate in the same binding lifecycle.
- Telegram emits compact `er1` callbacks and preserves old JSON callbacks only as unbound compatibility data. The complete callback survives parser, strict queue schemas, routing, worker handling, and application authorization. Sender mismatch, malformed versioned data, expired context, old revisions, pre-presentation text, and unresolved claims execute no review action.
- `expense-confirmation-context.e2e.spec.ts` replaces a 30 EUR review with a presented 35 EUR revision, rejects old confirm/cancel/correct buttons, ignores caller-spoofed expense data, saves the persisted 35 EUR amount once, and rejects a distinctly delivered duplicate callback.
- Exact-command tests cover the full allowlist and the required mixed, negated, concatenated, quoted, emoji, internal-punctuation, conditional, and prompt-injection negatives. Legacy re-presentation and failed-presentation tests prove authorization remains unavailable until delivery succeeds.
- The complete suite passed outside the socket/filesystem sandbox with one fork: 142 test files and 1,902 tests passed; 4 conditionally disabled suites and 29 tests remained skipped. The PostgreSQL/Redis conversation-state suites ran and passed. `pnpm test` itself remains unusable in this environment because pnpm reports `unable to open database file`; the equivalent project-local Vitest binary was used.
- Project-local `eslint .` and `tsc --noEmit` passed. ESLint emitted only the existing module-type performance warning.
...
- [x] Bind delayed undo presentation to its offered latest record, successful presentation timestamp, revision, and non-null expiry. Recheck the latest non-deleted record under the claimed execution before external deletion; preserve external-delete-before-local-success/audit ordering.
- [x] Preserve deterministic one-message immediate undo, including queued-review eligibility consumption, and prevent unrelated or stale callbacks from being converted into deletion authorization. No inferred undo execution is added.
- [x] Bind the exact retry command to the persisted confirmed expense and ten-minute window, atomically consume its single available reattempt, and preserve terminal manual fallback, transparent OAuth refresh limits, and reconfiguration policy. Legacy valid retry/undo context must be re-presented before accepting newly bound authorization if presentation cannot be established.
- [x] Add `financial-action-context.integration.spec.ts`: delayed undo at/after expiry before sweep; latest record replaced; immediate eligibility consumed once; concurrent confirms; duplicate retry; retry expiry; first append timeout followed by one explicit retry; second failure terminal; process restart with an unresolved claim; OAuth/timeout attempts during append/delete; local finalization failure after remote success. Assert no unauthorized external call, no second automatic call, and no false success/audit or overwrite of successor state.
- [x] Add a deferred-promise context-validation test representing a future model request: return after revision change or lease loss, reject the proposal, and assert zero state/queue/message/financial effects from it. Use no live provider or semantic dispatch integration.
- [x] Run existing evaluator offline regressions without modifying frozen labels merely to fit new behavior. If deterministic-policy reports change, version the policy/report evidence and explain the whole-message change separately from model accuracy; retain Phase 1 live-evaluation limitations.
- [x] Update undo, confirmation/retry, state-management and relevant evaluation docs, feature indexes, FSM payloads, and the concurrency ADR/index as applicable. Record coordinated deployment and safe rollback checks with queued legacy jobs and unresolved claims.
- [x] Run `pnpm test` and required PostgreSQL/Redis suites; attach scenario counts, zero false-authorization/unauthorized-effect observations, and remaining external verification to this plan and the master's implementation table. Do not mark master Phase 2 implemented until all three deliveries and migration verification are complete.
- [x] Run `pnpm run lint` and `pnpm run typecheck` to verify linting and typechecking. Fix issues if any.
- [x] Ask the user if they want to review the changes before continuing, or proceed directly with the next phase.

#### Completion evidence (2026-09-14)

- Delayed undo and explicit retry now persist separate action bindings before presentation, require a later channel timestamp, atomically consume the current offer into a financial claim, and reject expired, replaced, malformed, duplicate, or unresolved context before any provider mutation. Legacy and failed-delivery offers are re-presented without consuming the triggering command.
- `financial-action-context.integration.spec.ts` ran with real disposable PostgreSQL and Redis containers: 6 consolidated scenarios passed. The suite observed zero unauthorized append/delete calls, one winner for competing authorization, no claim replacement by timeout/OAuth-style writers, persistence across restart, and zero state/queue/message/financial effects from a deferred stale proposal. Unit, worker, and E2E scenarios additionally cover latest-record replacement, exact retry vocabulary, first known-no-effect append failure followed by one explicit retry, terminal second failure, OAuth refresh limits, and local-finalization failure after remote success.
- The complete suite ran with one fork and containers enabled: 144 test files and 1,924 tests passed; 4 unrelated conditionally disabled suites and 29 tests remained skipped. The required financial-action, conversation-state/concurrency, OAuth-token, and spreadsheet-access PostgreSQL/Redis suites ran and passed.
- Offline `semantic-corpus-v2` regressions preserved frozen `semantic-labels-v2`: development passed 175/175 checks and held-out passed 25/25, with zero fixture mismatches and zero critical-case failures. Lexical comparison counts were unchanged; the deterministic-source digest advanced to `418018387cebf748b8a33c88da94142133535c4451ba3895c76e4b827acf7d95`. No live provider or semantic dispatch was enabled.
- Project-local `eslint .` and `tsc --noEmit` passed. ESLint emitted only the existing module-type performance warning. No migration was applied to a user-configured database and no production rollout was performed; deployment still requires migration `0009`, coordinated worker draining, legacy offer re-presentation, and unresolved-claim inspection.

## Next step

Review the completed confirmation-safety prerequisite, then create the master semantic-router shadow-pipeline subplan when explicitly requested.
